...
const VALORES_PADRAO: FeiraFormValues = {
  nome: "",
  descricao: "",
  categorias: [],
  dataInicio: "",
...
type Props = {
  modo: "criar" | "editar"
  feiraId?: string
  valoresIniciais?: Partial<FeiraFormValues>
  gridBloqueado?: boolean
  gridBloqueadoMotivo?: string
}

export function FeiraFormWizard({
  modo,
  feiraId,
  valoresIniciais,
  gridBloqueado = false,
  ...
  const [dados, setDados] = useState<FeiraFormValues>({
    ...VALORES_PADRAO,
    ...valoresIniciais,
  })
  ...
  const voltarHref = modo === "editar" && feiraId ? `/feiras/${feiraId}` : "/dashboard/organizador"
  ...
  async function handleConfirmar() {
    ...
    try {
      ...
      const result =
        modo === "editar" && feiraId
          ? await editarFeiraAction(feiraId, payload)
          : await criarFeiraAction(payload)
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
            {modo === "editar" ? "Editar Feira" : "Nova Feira"}
          </h1>
          ...
          {step < 5 ? (
            ...
            >
              {loading
                ...
                : modo === "editar" ? "Salvar Alterações" : "Criar Feira"}
            </Button>
          )}
        </div>
      </div>
